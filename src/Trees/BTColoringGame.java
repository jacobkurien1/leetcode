...
/*
Running time O(n)
Space needed is O(n) as the binary tree can be skewed and the stack can grow to N
 */
public class BTColoringGame {
    int nodeCountUnderX = 0;
    int nodeCountUnderXLeft = 0;
    int nodeCountUnderXRight = 0;
    public boolean btreeGameWinningMove(TreeNode root, int n, int x) {
        getNodeCount(root, x);
        return (n - nodeCountUnderX > nodeCountUnderX) || // case when we select the parent of the initially colored node
                (n-nodeCountUnderXLeft < nodeCountUnderXLeft) || // case when we select the left child of x
                (n-nodeCountUnderXRight < nodeCountUnderXRight); // case when we select the right child of x
    }

    int getNodeCount(TreeNode n, int x){
        if(n == null){
            return 0;
        }
        int nodeCountUnderNLeft = getNodeCount(n.left, x);
        int nodeCountUnderNRight = getNodeCount(n.right, x);
        int nodeCountUnderN = nodeCountUnderNLeft + nodeCountUnderNRight + 1;
        if(n.val == x){
            nodeCountUnderX = nodeCountUnderN;
            nodeCountUnderXLeft = nodeCountUnderNLeft;
            nodeCountUnderXRight = nodeCountUnderNRight;
        }
        return nodeCountUnderN;
    }
    class TreeNode {
        int val;
        TreeNode left;
