...
The cities are connected by flights. The flights are represented as a N*N matrix (not necessary symmetrical), c
alled flights representing the airline status from the city i to the city j. If there is no flight from the city i to the city j, flights[i][j] = 0;
 ...
 */
/*
Space needed is O(cityCount)
Running time is O(cityCount^2*weekCount)
 */
public class MaxVacationDaysDuringWork {
    /*
   We will have a maxVac Matrix maxVac[i][j] to track the maximum vacation if we are in week i and city j
   maxVac[week][city] = Math.max for all m (maxVac[week-1][m] + days[city][week]) if flight[m][city] = 1
   */
    public int maxVacationDays(int[][] flights, int[][] days) {
        int cityCount = flights.length;
        int weekCount = days[0].length;
        int[] vacPrevWeek = new int[cityCount];
        Arrays.fill(vacPrevWeek, Integer.MIN_VALUE);
        vacPrevWeek[0] = 0;
        int lastWeekMaxVac = 0;
        for(int week = 0; week<weekCount; week++){
            int[] vacCurrWeek = new int[cityCount];
            lastWeekMaxVac = 0;
            for(int city = 0; city<cityCount; city++){
                int maxVac = Integer.MIN_VALUE;
                for(int m = 0; m < flights.length;m++){
                    if(vacPrevWeek[m] != Integer.MIN_VALUE && (flights[m][city] == 1 || m == city)){ // m==city to consider the case where we stay in the same city
                        maxVac = Math.max(maxVac, vacPrevWeek[m] + days[city][week]);
                    }
                }
                lastWeekMaxVac = Math.max(lastWeekMaxVac, maxVac);
                vacCurrWeek[city] = maxVac;
            }
            vacPrevWeek = vacCurrWeek;
        }
        return lastWeekMaxVac;
    }
}
