...
/*
Running time is O(n)
Space needed is O(1) as the Queue does not have a size greater than 4.
 */
public class ReadNCharsGivenRead4 {
    ...
    public int read(char[] buf, int n) {
        ...
        while(currentIndex <n){
            if(q.isEmpty()){
                ...
                int fromRead4 = read4(read4Buf);
                if(fromRead4 == 0){
                    return currentIndex;
                }
                for(int i = 0; i<fromRead4; i++){
                    q.add(read4Buf[i]);
                }
            }
            while(!q.isEmpty() && currentIndex <n){
                char c = q.poll();
                buf[currentIndex++] = c;
            }
        }
    ...
    //writing this to make this file compile
    int read4(char[] val){
        return -1;
    }
}
