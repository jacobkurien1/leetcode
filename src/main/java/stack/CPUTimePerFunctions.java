...
Space needed is O(n)
 */
public class CPUTimePerFunctions {

    public int[] exclusiveTime(int n, List<String> logs) {
        int[] timeForJob = new int[n];
        Stack<Interval> st = new Stack<>();
        int lastEnd = -1;
        for(String log : logs){
            Interval curr = new Interval(log);
            if(curr.isStart){
                if(!st.isEmpty()){
                    timeForJob[st.peek().id] += curr.time - st.peek().time;
                }
                st.push(curr);
            } else {
                Interval start = st.pop();
                timeForJob[start.id] += curr.time - ((lastEnd == -1)?start.time -1:lastEnd);
                lastEnd = curr.time;
            }
        }
        return timeForJob;
    ...
    class Interval {
        ...
        public Interval(String log){
            String[] logParts = log.split(":"); // checks to verify log
            id = Integer.parseInt(logParts[0]);
            isStart = logParts[1].equals("start");
            time = Integer.parseInt(logParts[2]);
        }
    }
}
