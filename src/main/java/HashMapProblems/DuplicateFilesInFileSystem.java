 ...
 */
/*
Running time is O(n * x) where n strings of x length are present
Space needed is O(n*x) used by the hashmap
 */
public class DuplicateFilesInFileSystem {
    public List<List<String>> findDuplicate(String[] paths) {
        ...
        for(String path: paths){
            ...
            for(int i = 1; i<parts.length; i++){
                ...
                List<String> pathList = hm.getOrDefault(file[1], new ArrayList<String>());
                pathList.add(parts[0]+"/"+file[0]);
                hm.put(file[1], pathList);
            }
        }

        for(List<String> pathGroup : hm.values()){
            if(pathGroup.size()>1){
                ret.add(pathGroup);
            }
        }
