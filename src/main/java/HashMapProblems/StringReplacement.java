 ...
 */
/*
Running time is O(s.length()* numberOfReplacements) for constructing the output
Space needed is O(n) considering the target length is constant.
 */
public class StringReplacement {
    public String findReplaceString(String S, int[] indexes, String[] sources, String[] targets) {
        HashMap<Integer, ReplacementString> hm = new HashMap<>();
        for(int i = 0; i<indexes.length; i++){
            if(i+sources[i].length() <= S.length() &&
                    S.substring(indexes[i], indexes[i]+sources[i].length()).equals(sources[i])){
                hm.put(indexes[i], new ReplacementString(targets[i], sources[i].length()));
            }
        }
        ...
        while(i<S.length()){
            if(hm.containsKey(i)){
                ReplacementString rep = hm.get(i);
                sb.append(rep.stringReplacement);
                i+= rep.lengthToReplace;
            }else {
    ...
    class ReplacementString{
        ...
        int lengthToReplace;
        ReplacementString(String stringReplacement, int lengthToReplace){
