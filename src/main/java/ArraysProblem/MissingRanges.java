...
public class MissingRanges {
    public List<String> findMissingRanges(int[] nums, int lower, int upper) {
        ...
        if(nums[nums.length-1] != upper){
            addRange(missingRanges, nums[nums.length-1]+1, upper);
        }
        return missingRanges;
    }

    void addRange(List<String> missingRanges, int min, int max){
        if(min == max){
            missingRanges.add(Integer.toString(min));
        } else {
            missingRanges.add(Integer.toString(min)+"->"+Integer.toString(max));
        }
    }
}
